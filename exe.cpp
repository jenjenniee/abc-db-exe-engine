...
struct Table {
    ...
    std::vector<Page*> pages;          // 테이블이 보유한 페이지
...
public:
    ...
    void insertInto(const std::string& tableName, const std::string& record) {
        std::unordered_map<std::string, Table*>::iterator it = tables.find(tableName);
        if (it == tables.end()) {
...
public:
    ...
    // Next 함수 규칙 반영 
    char* Next() override {
        // 현재 페이지가 비어있거나, 모든 레코드를 처리한 경우 다음 페이지로 이동
        while (currentPage == nullptr || currentRecordIndex >= currentPage->numRecords) {
            ...
            // 더 이상 페이지가 없으면 null 반환
            if (currentPage == nullptr) {
                return nullptr;
            }

            // 새로운 페이지로 이동하면 인덱스 초기화
...
// 테이블의 모든 레코드를 순차적으로 읽기, 테이블의 모든 페이지를 순차적으로 스캔하면서 레코드를 반환
// class TableScanOperator : public Operator {
//     DiskManager* diskManager;
//     int currentRecordIndex;
//     int currentPageNum;
